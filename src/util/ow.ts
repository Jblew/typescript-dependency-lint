...
export const owFileExists = ow.string.minLength(1).is(filePath => {
  return (
    fs.existsSync(filePath) || `Expected ${filePath} to be a file that exists`
  )
})

export const owParentDirExists = ow.string.minLength(1).is(filePath => {
...
export const owIValidJson = ow.string.minLength(1).is(v => {
  try {
    JSON.parse(v)
    return true
  } catch (error) {
